...
public class CodebookLibrary
{
    ...
    public CodebookLibrary()
    {
        ...
        RandomAccessReader raf = new RandomAccessReader(codebookFile, ByteOrder.LITTLE_ENDIAN);

        long streamSize = raf.remaining();

        raf.seek(streamSize - 4);
        int offsetOffset = raf.readInt();

        codebookCount = (int) (streamSize - offsetOffset) / 4;
